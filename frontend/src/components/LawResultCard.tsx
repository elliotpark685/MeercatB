...
import ArticleDetailPopover from './ArticleDetailPopover';
...
export default function LawResultCard({
  item,
  onViewDetail,
  isDetailOpen = false,
  onCloseDetail,
  ...
  return (
    <article className="group relative rounded-2xl border border-[#2C2C2E] bg-[#1A1A1A] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#3A3A3C]">
      ...
      {isDetailOpen && onCloseDetail && (
        <ArticleDetailPopover articleId={item.article_id} onClose={onCloseDetail} />
      )}
    </article>
  );
}
