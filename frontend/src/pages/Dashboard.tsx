...
function StatCard({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div className="bg-[#1E1E1E] rounded-2xl border border-[#2C2C2E] p-5 hover:border-[#3A3A3C] transition-colors">
      <p className="text-xs font-medium text-[#98989D] uppercase tracking-widest">
        {label}
      </p>
      <p
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {isFirstLoad ? (
          ...
          <>
            <StatCard
              label="생성 문서 수"
              value={data.total_generated_documents}
            />
      ...
      {isFirstLoad ? (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          {[0, 1].map((i) => (
            <div
              key={i}
              className="bg-[#1E1E1E] rounded-2xl border border-[#2C2C2E] p-5"
            >
          ...
          <div className="bg-[#1E1E1E] rounded-2xl border border-[#2C2C2E] p-5">
            ...
            {data.latest_generated_documents.length === 0 ? (
              <EmptyState
                icon="📄"
                title="생성된 문서가 없습니다"
                description="문서 생성 메뉴에서 첫 번째 문서를 만들어보세요."
              />
            ) : (
                ...
                {data.latest_generated_documents.map((doc) => (
                  ...
                  >
                    <div className="min-w-0">
                      <span className="font-medium text-white truncate block max-w-[160px]">
                        {doc.title}
                      </span>
          ...
          <div className="bg-[#1E1E1E] rounded-2xl border border-[#2C2C2E] p-5">
            ...
            {data.latest_law_searches.length === 0 ? (
              <EmptyState
                icon="⚖️"
                title="검색 기록이 없습니다"
                description="법령 검색 메뉴에서 관련 법령을 조회할 수 있습니다."
              />
            ) : (
              <ul className="space-y-0">
