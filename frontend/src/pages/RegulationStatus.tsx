...
const dateText = (value: string | null) => {
  if (!value) return '정보 미수집';
  const [year, month, day] = value.split('-');
  return year && month && day ? `${year}.${month}.${day}` : value;
};
const dateRank = (value: string | null) => (value ? new Date(value).getTime() : 0);
const isScheduled = (value: string | null) => Boolean(value && value > new Date().toISOString().slice(0, 10));

function RegulationStatusBadge({ effectiveDate }: { effectiveDate: string | null }) {
  const scheduled = isScheduled(effectiveDate);
  return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${scheduled ? 'bg-orange-400/10 text-orange-300' : 'bg-green-400/10 text-green-300'}`}>
    {scheduled ? '시행 예정' : '시행 중'}
  </span>;
}
...
export default function RegulationStatus() {
  ...
  return <section className="mx-auto max-w-7xl space-y-6 text-[#F5F5F7]">
    ...
    <div className="grid gap-3 sm:grid-cols-3"><SummaryCard label="전체 문서" value={items.length} detail="현재 활성 문서" accent="text-white" /><SummaryCard label="법령" value={lawCount} detail="법·시행령·시행규칙" accent="text-cyan-300" /><SummaryCard label="안전기준" value={standardCount} detail={latest ? `최신 개정 ${dateText(latest)}` : '개정일 수집 중'} accent="text-violet-300" /></div>
    <div className="rounded-2xl border border-[#2C2C2E] bg-[#1E1E1E]">
      ...
      {loading ? <div className="p-12 text-center text-sm text-[#98989D]">현황 정보를 불러오는 중입니다.</div> : error ? <div className="p-12 text-center text-sm text-[#FF9F0A]">{error}</div> : <><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[980px] text-left"><thead className="bg-[#191919] text-xs font-medium text-[#98989D]"><tr><th className="px-5 py-3">문서명</th><th className="px-4 py-3">구분</th><th className="px-4 py-3">공포·발령번호</th><th className="px-4 py-3">시행일</th><th className="px-4 py-3">상태</th><th className="px-4 py-3">최종 개정일</th><th className="px-4 py-3">수록 조문</th><th className="px-5 py-3 text-right">원문</th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id} className="border-t border-[#2C2C2E] text-sm transition hover:bg-[#252525]"><td className="px-5 py-4"><div className="font-medium text-[#F5F5F7]">{item.name}</div><div className="mt-1 text-xs text-[#8E8E93]">{item.document_type || item.source_type || item.provider || '문서 유형 미수집'}</div></td><td className="px-4 py-4"><CategoryBadge category={item.category} /></td><td className="px-4 py-4 text-[#D1D1D6]">{item.law_no || '정보 미수집'}</td><td className="px-4 py-4 text-[#D1D1D6]">{dateText(item.effective_date)}</td><td className="px-4 py-4"><RegulationStatusBadge effectiveDate={item.effective_date} /></td><td className="px-4 py-4 text-[#D1D1D6]">{dateText(item.latest_amendment_date)}</td><td className="px-4 py-4 text-[#D1D1D6]">{item.article_count.toLocaleString()}개</td><td className="px-5 py-4 text-right">{item.source_url ? <a href={item.source_url} target="_blank" rel="noreferrer" className="text-[#00E5FF] hover:underline">열기 ↗</a> : <span className="text-[#636366]">미연결</span>}</td></tr>)}</tbody></table></div><div className="space-y-3 p-4 md:hidden">{filtered.map((item) => <article key={item.id} className="rounded-xl border border-[#303033] bg-[#191919] p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="font-medium">{item.name}</h2><p className="mt-1 text-xs text-[#8E8E93]">{item.law_no || '번호 정보 미수집'}</p></div><CategoryBadge category={item.category} /></div><div className="mt-3"><RegulationStatusBadge effectiveDate={item.effective_date} /></div><dl className="mt-4 grid grid-cols-2 gap-3 text-xs"><div><dt className="text-[#8E8E93]">시행일</dt><dd className="mt-1 text-[#D1D1D6]">{dateText(item.effective_date)}</dd></div><div><dt className="text-[#8E8E93]">최종 개정일</dt><dd className="mt-1 text-[#D1D1D6]">{dateText(item.latest_amendment_date)}</dd></div></dl>{item.source_url && <a href={item.source_url} target="_blank" rel="noreferrer" className="mt-4 inline-block text-sm text-[#00E5FF]">원문 열기 ↗</a>}</article>)}</div>{!filtered.length && <div className="border-t border-[#2C2C2E] p-12 text-center text-sm text-[#98989D]">조건에 맞는 문서가 없습니다.</div>}</>}
    </div>
  </section>;
}
