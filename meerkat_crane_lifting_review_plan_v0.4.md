# MeerkatAI 크레인 양중 검토 기능 기획·설계 정리 v0.4
...
v0.3에 대한 구현 전 검토 의견을 반영하여 **현재 Parser PoC와 향후 Engineering/Planner 기능의 경계를 명확히 확정**했다.

- 현재 단계는 **Terex TRT60 Parser Vertical Slice PoC**로 한정
- 현재 PoC에서는 Geometry / Reach / Capacity Review / UI를 구현하지 않음
- `Required Height`는 향후 Engineering Review에서 **사용자가 직접 입력하는 필수 작업조건**으로 확정
- 시스템은 Required Height를 자동 추론·보정하지 않음
- 향후 단면도의 주목적은 Height 산출이 아니라 **Boom/Jib ↔ Building Clearance/Interference 검토**
- Load Chart의 `capacity_basis`와 제조사별 Deduction/Adjustment 근거를 명시적으로 관리
- Configuration 조건은 상위 `LiftingConfiguration / LoadChart`에 유지하고 Cell에 중복 저장하지 않음
- `LoadChartCell`에 `cell_status`와 `source_text`를 추가하여 빈 셀·금지·적용불가·파싱오류를 구분
- Parser Verification / User Confirmation / Engineer Approval 상태를 분리
- Parser PoC의 Critical Error 목표는 **0건**
- PDF에서 추출하는 수치는 원문 값과 **정확 일치**를 원칙으로 함
- 동일 PDF + 동일 Parser Version은 동일 Canonical Data를 생성해야 함
- 단계 명칭을 `Parser PoC / Engineering Review PoC / Interactive Planner PoC`로 분리
...
`rated_capacity_t = null` 하나만으로 빈 셀, 금지 셀, 적용불가, 파싱실패를 혼동하지 않는다.
...
rated_capacity_t = null
```

내부 계산값은 SI 단위로 정규화하되, 원문의 수치와 단위를 함께 보존한다.

Counterweight / Outrigger / Working Area 등 Chart 전체 Configuration 조건은 Cell마다 중복 저장하지 않고 상위 `LiftingConfiguration / LoadChart` 관계로 추적한다. Parts of Line은 제조사 문서에서 Chart-level인지 Cell-level인지 확인한 후 해당 Parser Profile에서 모델링한다.
...
Load Chart의 정격하중이 무엇을 포함하는지 제조사 원문 근거로 관리한다.
...
- Hook Block, Sling, Spreader, Wire Rope, Stowed Jib 등과의 관계를 임의 추정하지 않음
- `UNKNOWN`이면 향후 Engineering Review에서 자동 Capacity 판정을 차단
- 동일 하중요소가 Gross Load와 Deduction 양쪽에서 중복 적용되지 않도록 한다

`capacity_basis`는 Load Chart 또는 해당 Configuration 수준의 속성으로 관리하며 Cell마다 반복 저장하지 않는다.
...
이 기능들은 Parser PoC 성공 이후 단계에서 구현한다.

### 23.3 Parser PoC 성공 기준
...
PDF에 표시된 수치를 추출하는 Parser 검증에서는 임의 수치 허용오차를 적용하지 않는다.
...
Geometry 계산 등에서 발생하는 부동소수점 허용오차는 향후 Engineering Review PoC에서 별도 정의한다.
...
## 27. v0.4 구현 전 최종 확정사항

1. **현재 구현은 TRT60 Parser PoC다.**
2. Parser PoC에서는 Height / Geometry / Capacity Review를 계산하지 않는다.
3. 향후 Engineering Review에서 **Required Height는 사용자가 직접 입력한다.**
4. 시스템은 Required Height를 자동 추론하거나 보정하지 않는다.
5. 단면도는 향후 Required Height 산출용이 아니라 **Boom/Jib와 구조물의 Clearance/Interference 검토용**이다.
6. Load Chart 수치는 Configuration과 분리될 수 없으며 Configuration 연결 오류는 Critical Error다.
7. `capacity_basis`와 Deduction/Adjustment는 제조사 원문 근거가 있을 때만 적용한다.
8. Chart-level Configuration을 모든 Cell에 중복 저장하지 않는다.
9. Cell의 빈 값/금지/적용불가/파싱오류 상태를 명시적으로 구분한다.
10. Parser Verification, User Confirmation, Engineer Approval은 별도 상태 체계다.
11. Parser PoC의 Critical Error 허용 목표는 **0건**이다.
12. 동일 PDF + 동일 Parser Version은 동일 Canonical Data를 생성해야 한다.
