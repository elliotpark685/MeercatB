"""Create a blank Human Golden CSV for TRT35 page-15 8 m lattice jib / 20 deg."""
...
FIELDNAMES = [
    "source_document_hash", "source_page", "configuration", "row_index", "column_index",
    "radius_m", "boom_length_m", "expected_cell_status", "expected_capacity_t",
    "source_bbox_x0", "source_bbox_y0", "source_bbox_x1", "source_bbox_y1",
    "human_verified", "verified_at", "verification_note",
]
...
def main() -> None:
    ...
    if result.grid_status != "PASS" or len(result.cells) != 160:
        raise ValueError("page-15 lattice jib / 20 deg grid must resolve to exactly 160 cells")
